 ...
 * Handles uploading documents to the document storage API
 */
...
export const getDocumentType = (file: File | string): string => {
  ...
  if (typeof file !== 'string') {
    ...
    return getDocumentTypeFromName(file.name);
  }

  // If a filename/URL string is provided
  return getDocumentTypeFromName(file);
};

/**
 * Get document type from filename or URL
 */
export const getDocumentTypeFromName = (fileNameOrUrl: string): string => {
