...
interface UserStore {
  ...
  requestPasswordReset: (data: { email?: string; phone_number?: string }, headers?: Record<string, string>) => Promise<void>;
  confirmPasswordReset: (data: { email?: string; phone_number?: string; reset_token: string; new_password: string }, headers?: Record<string, string>) => Promise<void>;
  ...
  (set, get) => ({
    ...
    updateUser: async (
      ...
      try {
        ...
        const response = await apiClient.put<any>(`/users/${id}`, data, headers);
    ...
    requestPasswordReset: async (data: { email?: string; phone_number?: string }, headers?: Record<string, string>) => {
      const { setActiveAction, setLoading, setStoreError } = get();
      try {
        setActiveAction('update');
        setLoading(true);

        await apiClient.post<any>('/auth/password/reset/request', data, headers);
    ...
    confirmPasswordReset: async (data: { email?: string; phone_number?: string; reset_token: string; new_password: string }, headers?: Record<string, string>) => {
      const { setActiveAction, setLoading, setStoreError } = get();
      try {
        setActiveAction('update');
        setLoading(true);

        await apiClient.post<any>('/auth/password/reset/confirm', data, headers);

        setLoading(false);
      } catch (error: unknown) {
        const errorMessage = error instanceof Error ? error.message : 'Failed to confirm password reset';
        const errorStatus = (error as any)?.response?.status;
        setStoreError({
          message: errorMessage,
          status: errorStatus,
