...
export const productFormSchema = z.object({
    ...
    sku: z.string().min(1, { // Base SKU for the product
    ...
    sale_price: z.coerce.number().min(0.01, "Sale price must be greater than 0.").optional().nullable(), // Made optional and nullable
    cost_price: z.coerce.number().min(0).optional(),
    inventory_quantity: z.coerce.number().int().min(0, { // Overall stock if not tracking by variants or if has_variants is false
    ...
    (data) => { // Refinement for inventory quantity when not tracking by variants
        if (!data.has_variants && data.inventory_tracking && typeof data.inventory_quantity !== 'number') {
            return false; // If not has_variants and tracking inventory, inventory_quantity is required
        }
        return true;
    },
    {
        message: "Inventory quantity is required when tracking stock for a product without variants.",
